(() => {
        ...
        window.addEventListener("load", (function(e) {
            ...
            function getHeight(showMoreBlock, showMoreContent) {
                ...
                if (showMoreType === "items") {
                    ...
                    const showMoreItems = showMoreContent.querySelector("ul").children;
                    for (let index = 0; index < showMoreItems.length; index++) {
                        const showMoreItem = showMoreItems[index];
                        if (index >= showMoreTypeValue) showMoreItem.style.display = "none"; else {
                            const marginTop = parseFloat(getComputedStyle(showMoreItem).marginTop) ? parseFloat(getComputedStyle(showMoreItem).marginTop) : 0;
                            const marginBottom = parseFloat(getComputedStyle(showMoreItem).marginBottom) ? parseFloat(getComputedStyle(showMoreItem).marginBottom) : 0;
                            hiddenHeight += showMoreItem.offsetHeight + marginTop;
                            if (index < showMoreTypeValue - 1) hiddenHeight += marginBottom;
                        }
                    }
    ...
    function rangeInit() {
        ...
        document.addEventListener("DOMContentLoaded", (function() {
            ...
            rangeBoxes.forEach((rangeBox => {
                const slider = rangeBox.querySelector(".slider-range");
                const fromInput = rangeBox.querySelector(".from-price");
                const toInput = rangeBox.querySelector(".to-price");
                if (!slider || !fromInput || !toInput) {
                    console.log("Missing elements in .range-box");
                    return;
                }
                initialize(slider, {
                    start: [ 1e3, 5e3 ],
                ...
                slider.noUiSlider.on("update", (function(values, handle) {
                    if (handle === 0) fromInput.value = values[0]; else toInput.value = values[1];
                }));
    ...
    function initSliders() {
        if (document.querySelector(".swiper")) {
            new swiper_core_Swiper(".partners-main__slider", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                slidesPerView: 3,
            ...
            new swiper_core_Swiper(".slide-certificates__slider", {
    ...
    const toggleCatalogMenu = () => {
        ...
        if (!catalogButtons.length) {
            console.error("No catalog buttons found!");
            return;
        }
        if (!catalogMenu) {
            console.error("No catalog menu found!");
            return;
        }
        const closeMenu = () => {
        ...
        const openMenu = button => {
            ...
            catalogMenu.style.top = `${buttonRect.top + scrollY + 60}px`;
            catalogMenu.classList.add("open");
            htmlElement.classList.add("catalog-open");
        };
        const handleClick = event => {
            const button = event.target.closest(".button-catalog");
            if (button) {
                if (catalogMenu.classList.contains("open") && catalogMenu.style.top === `${button.getBoundingClientRect().top + window.scrollY + 60}px`) closeMenu(); else openMenu(button);
                return;
            }
            if (!event.target.closest(".catalog-menu")) closeMenu();
        };
        document.addEventListener("click", handleClick);
    };
    const toggleCatalogFilter = () => {
        const catalogButtons = document.querySelectorAll(".button-filter");
        const catalogMenu = document.querySelector(".sidebar-catalog");
        const htmlElement = document.documentElement;
        if (!catalogButtons.length) {
            console.error("No catalog buttons found!");
            return;
        ...
        const handleClick = event => {
            ...
            if (!event.target.closest(".sidebar-catalog")) closeMenu();
        };
        document.addEventListener("click", handleClick);
    };
    const resetFilters = () => {
        document.addEventListener("DOMContentLoaded", (() => {
            const filters = document.querySelectorAll(".options-filter");
            if (filters.length === 0) {
                console.log("No .options-filter elements found.");
                return;
            }
            filters.forEach((filter => {
                const resetButton = filter.querySelector(".reset-filters");
                if (!resetButton) {
                    console.log("No .reset-filters button found within .options-filter.");
                    return;
                }
